...
function selectareTest($categorieTest)
{
    include_once 'bd.php';
    ...
    $sqlCountMax = "SELECT max(id) FROM public.exercises WHERE categorie = '$categorie'"; // WHERE is_verified <> 0";
    $rs = pg_query($db, $sqlCountMax);
    $randNumberMax = intval(pg_fetch_row($rs)[0]);
    //echo "randNbMax".$randNumberMax.' ';
    $exercitiiVerificate = array();
    $sqlCountMax = "SELECT id FROM public.exercises WHERE is_verified = 1 AND categorie = '$categorie'";
    $rs = pg_query($db, $sqlCountMax);
    ...
    for ($i = 1; $i <= 5; $i++) {
        ...
        //echo $randNumber;
        $sql = "SELECT * FROM public.exercises WHERE id=$1 AND categorie = $2";
        $rs = pg_query_params($db, $sql, array($randNumber, $categorie)) or die("Cannot execute query: $sql\n");
